...
class Issue(HelixBase, AuditMixin, Base):
    ...
    sequence_id: Mapped[int] = mapped_column(Integer, nullable=False, comment="Per-project auto-increment (HLX-1, HLX-2...)")
    ...
    # Classification
    priority: Mapped[str] = mapped_column(String(20), default=IssuePriority.NONE, nullable=False, index=True)
    state_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("issue_states.id", ondelete="RESTRICT"), nullable=False)
    ...
    # Hierarchy (subtasks)
    parent_id: Mapped[uuid.UUID | None] = mapped_column(UUID(as_uuid=True), ForeignKey("issues.id", ondelete="CASCADE"), nullable=True, index=True)
    ...
    assignees: Mapped[list["IssueAssignee"]] = relationship("IssueAssignee", back_populates="issue", cascade="all, delete-orphan", lazy="selectin")
    ...
    activities: Mapped[list["Activity"]] = relationship("Activity", back_populates="issue", cascade="all, delete-orphan")
...
class Comment(HelixBase, AuditMixin, Base):
    ...
    __table_args__ = (Index("ix_comments_issue", "issue_id"),)
...
class Activity(HelixBase, Base):
    ...
    activity_type: Mapped[str] = mapped_column(String(30), default="updated", nullable=False, comment="created | updated | commented | deleted")
