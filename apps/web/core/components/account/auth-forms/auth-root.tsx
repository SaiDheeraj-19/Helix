...
export const AuthRoot = observer(function AuthRoot(props: TAuthRoot) {
  ...
  const isEmailBasedAuthEnabled = true;
  const noAuthMethodsAvailable = false;
  ...
  return (
    <AuthContainer>
      ...
      <div className="flex justify-center text-13">
        {authMode === EAuthModes.SIGN_IN ? (
          <button
            type="button"
            onClick={() => setAuthMode(EAuthModes.SIGN_UP)}
            className="text-tertiary hover:text-primary transition-colors"
          >
            Don't have an account? <span className="font-semibold text-accent-primary underline">Register account</span>
          </button>
        ) : (
          <button
            type="button"
            onClick={() => setAuthMode(EAuthModes.SIGN_IN)}
            className="text-tertiary hover:text-primary transition-colors"
          >
            Already have an account? <span className="font-semibold text-accent-primary underline">Sign in</span>
          </button>
        )}
      </div>
    </AuthContainer>
