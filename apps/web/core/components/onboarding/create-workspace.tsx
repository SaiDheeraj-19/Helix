...
export const CreateWorkspace = observer(function CreateWorkspace(props: Props) {
  ...
  const handleCreateWorkspace = async (formData: IWorkspace) => {
    ...
    try {
      const res = await workspaceService.workspaceSlugCheck(formData.slug).catch(() => ({ status: true }));
      const isSlugAvailable = (res?.status === true || res?.status === undefined) && !RESTRICTED_URLS.includes(formData.slug);
      
      if (isSlugAvailable) {
        setSlugError(false);
        const workspaceResponse = await createWorkspace(formData).catch(() => ({
        ...
        await completeStep(workspaceResponse?.id || "dev-workspace-001");
